...
#input-----------
inds_raw <- read.csv("output/indices.csv", row.names = 1)

#defining factor variables --------
source('R-scripts/factors.R') #recalling factors 
inds1<- inds_raw%>%
  select(S,AMBI,BENTIX,M.AMBI)
inds1[inds1$S == 1,]$AMBI<- 7
#all.equal(rownames(factors), rownames(inds1))
#factors <- factors[rownames(inds1),] #updating rows of factors with the deletion 
...
for (i in seq(ncol(inds1))){
  ...
  ST<- data.frame(index = inds1[,i],station = factors$station,
    ...
    summarise(mean = mean(index),SE = sd(index)/sqrt(n()))
...
plotSAH<- function(index= "AMBI", f1 = "season", f2 = "area", f3 = "habitat"){
  ...
  bar<- 
    ggplot(ind, aes(factor, mean,fill = fill))+
    geom_col(position = "dodge",col = "black")+ #by dodge the fill factor will be side to side instead of stacked
    geom_hline(yintercept = 1.2, linetype = "dashed", color = "gray23")+
    geom_hline(yintercept = 3.3, linetype = "dashed", color = "gray23")+
    geom_hline(yintercept = 4.3, linetype = "dashed", color = "gray23")+
    geom_hline(yintercept = 5.5, linetype = "dashed", color = "gray23")+
    ylim(0,7)+
...
# pairs.panels(inds1, gap=0,pch=20,method="spearman", 
